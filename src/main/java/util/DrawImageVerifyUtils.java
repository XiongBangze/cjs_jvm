...
public class DrawImageVerifyUtils {
	
	public String drawImageVerify(BufferedImage bi, int width, int height){
		Graphics g = bi.getGraphics();	// 获得画布
		g.setColor(Color.WHITE);		// 设置颜色
		g.fillRect(0, 0, width, height);// 填充区域
		g.setColor(Color.BLUE);			// 设置边框颜色
		g.drawRect(1, 1, width - 2, height - 2);		// 边框区域
		drawRandomLine(g, width, height);				// 画干扰线
	...
	private String drawRandomNum(Graphics2D g) {
		...
		g.setFont(new Font("SimSun", Font.BOLD, 20));	// 设置字体
...
//				+ "\u822c\u62a5\u53f6\u538b\u6162\u53d4\u80cc\u7ec6";
		...
		for (int i = 0; i < 4; i++) {
			...
			String ch = base.charAt(new Random().nextInt(base.length())) + "";	// 截取汉字
			sb.append(ch);
			g.rotate(degree * Math.PI / 180, x, 20);	// 正向角度
			g.drawString(ch, x, 20);
			g.rotate(-degree * Math.PI / 180, x, 20);	// 反向角度
			x += 30;
		}
		return sb.toString(); 
	}
